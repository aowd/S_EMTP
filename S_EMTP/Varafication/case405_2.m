...
idx = 1;
t_tmp = t_PSC(idx);
while abs(t_tmp - t_CPP(1))>1e-6
    idx = idx + 1;
    t_tmp = t_PSC(idx);
end
startIdx = idx;
while abs(t_tmp - t_CPP(end))>1e-6
    idx = idx + 1;
    t_tmp = t_PSC(idx);
end
endIdx = idx;
t_PSC = t_PSC(startIdx:endIdx);
...
t_PSC = t_PSC(int32(end/2):end);
t_CPP = t_CPP(int32(end/2):end);
...
% plot(t_PSC,Pac_psc,t_CPP,Pac_cpp)

idc1_mean_psc = mean(idc1_psc)
idc2_mean_psc = mean(idc2_psc)

udc1_mean_psc = mean(udc1_psc)
udc2_mean_psc = mean(udc2_psc)
...
idc1_f_psc = filter(Num_dc_1us,1,idc1_psc);
idc1_f_cpp = filter(Num_dc_5us,1,idc1_cpp);

plot(t_PSC,idc1_f_psc,t_CPP,idc1_f_cpp)

load Num_sw_5us.mat
load Num_sw_1us.mat

vra_psc_sw_f = filter(Num_sw_1us,1,vra_psc);
vrb_psc_sw_f = filter(Num_sw_1us,1,vrb_psc);
vrc_psc_sw_f = filter(Num_sw_1us,1,vrc_psc);
ira_psc_sw_f = filter(Num_sw_1us,1,ira_psc);
irb_psc_sw_f = filter(Num_sw_1us,1,irb_psc);
irc_psc_sw_f = filter(Num_sw_1us,1,irc_psc);
udc1_psc_sw_f = filter(Num_sw_1us,1,udc1_psc);
udc2_psc_sw_f = filter(Num_sw_1us,1,udc2_psc);
idc1_psc_sw_f = filter(Num_sw_1us,1,idc1_psc);
idc2_psc_sw_f = filter(Num_sw_1us,1,idc2_psc);

Pac_psc_sw_f = vra_psc_sw_f.*ira_psc_sw_f + vrb_psc_sw_f.*irb_psc_sw_f + vrc_psc_sw_f.*irc_psc_sw_f;
Pdc_psc_sw_f = udc1_psc_sw_f.*idc1_psc_sw_f + udc2_psc_sw_f.*idc2_psc_sw_f;
Ploss_psc_sw_f = 0.001*(ira_psc_sw_f.^2+irb_psc_sw_f.^2+irc_psc_sw_f.^2);

Pac_mean_psc_sw_f = mean(Pac_psc_sw_f(int32(end/2):end))
Pdc_mean_psc_sw_f = mean(Pdc_psc_sw_f(int32(end/2):end))
Ploss_mean_psc_sw_f = mean(Ploss_psc_sw_f(int32(end/2):end))

vra_cpp_sw_f = filter(Num_sw_1us,1,vra_cpp);
vrb_cpp_sw_f = filter(Num_sw_1us,1,vrb_cpp);
vrc_cpp_sw_f = filter(Num_sw_1us,1,vrc_cpp);
ira_cpp_sw_f = filter(Num_sw_1us,1,ira_cpp);
irb_cpp_sw_f = filter(Num_sw_1us,1,irb_cpp);
irc_cpp_sw_f = filter(Num_sw_1us,1,irc_cpp);
udc1_cpp_sw_f = filter(Num_sw_1us,1,udc1_cpp);
udc2_cpp_sw_f = filter(Num_sw_1us,1,udc2_cpp);
idc1_cpp_sw_f = filter(Num_sw_1us,1,idc1_cpp);
idc2_cpp_sw_f = filter(Num_sw_1us,1,idc2_cpp);

Pac_cpp_sw_f = vra_cpp_sw_f.*ira_cpp_sw_f + vrb_cpp_sw_f.*irb_cpp_sw_f + vrc_cpp_sw_f.*irc_cpp_sw_f;
Pdc_cpp_sw_f = udc1_cpp_sw_f.*idc1_cpp_sw_f + udc2_cpp_sw_f.*idc2_cpp_sw_f;
Ploss_cpp_sw_f = 0.001*(ira_cpp_sw_f.^2+irb_cpp_sw_f.^2+irc_cpp_sw_f.^2);

Pac_mean_cpp_sw_f = mean(Pac_cpp_sw_f(int32(end/2):end))
Pdc_mean_cpp_sw_f = mean(Pdc_cpp_sw_f(int32(end/2):end))
Ploss_mean_cpp_sw_f = mean(Ploss_cpp_sw_f(int32(end/2):end))