...
Iabc_PSC = result_PSC(:,7:9);
Iabc_CPP = result_CPP(:,7:9);
...
delta_Udc = abs( ( Udc_CPP_filter(start:end) - Udc_PSC_filter(start:end) ) ./ Udc_PSC_filter(start:end) );
...
delta_Mag = abs( ( Mag_CPP_filter(start:end) - Mag_PSC_filter(start:end) ) ./ Mag_PSC_filter(start:end) );
...
Err_phase = max(abs(delta_phase(start:end)))
...
Err_iabc = max(TVE(start:end))*100
...
i0_PSC = (1/3)*(result_PSC(:,10)+result_PSC(:,11)+result_PSC(:,12));
i0_CPP = (1/3)*(result_CPP(:,10)+result_CPP(:,11)+result_CPP(:,12));
figure(6)
plot(t,i0_PSC,t,i0_CPP)


figure(10)
plot(t,result_CPP(:,13));
