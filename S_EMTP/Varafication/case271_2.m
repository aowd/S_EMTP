...
result_PSC = result_PSC(idx:steps:idx+size(result_CPP,1)*steps-1,:);

max(abs(result_CPP-result_PSC))/1000

%% 选择需要观察的波形
% Udc_PSC = result_PSC(:,4)-result_PSC(:,5);
% Udc_CPP = result_CPP(:,4)-result_CPP(:,5);
% 
% % Udc_PSC = result_PSC(:,13);
% % Udc_CPP = result_CPP(:,13);
% 
...
% delta_Udc = abs( ( Udc_CPP_filter(5000:end) - Udc_PSC_filter(5000:end) ) ./ Udc_PSC_filter(5000:end) );
...
% delta_Mag = abs( ( Mag_CPP_filter(5000:end) - Mag_PSC_filter(5000:end) ) ./ Mag_PSC_filter(5000:end) );
...
% Err_phase = max(abs(delta_phase(5000:end)))
% 
% % 实部虚部计算
