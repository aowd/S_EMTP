...
/////////////////////////////////////////
	...
	for (int i = 1; i <= nBranch; i ++)
	{
		...
		tempCom->initializeBranch(initialVoltageVec,initialCurrentVec,ptr,curTime);
	}
	...
	{
		Component * tempCom = branches->at(i-1);
		tempCom->initializeBranch(nodeVoltageVec,branchCurrentVec,ptr,curTime);
	}
#endif
	...
	if (nIndGen != 0)
	{
		...
		// ===============================================
		//	若需要读入外界的风速文件，则需定义此宏
		//	读入文件的名称为caseXXX_GenVWData_0?.out。
		// 其第一列为时间，其余各列分别为每台风机的风速输入，其需要同仿真步长相同
		//	 By Gao Haixiang, 2/4/2013
		//================================================
#ifdef WIND_VELOCITY_DATA_INPUT
		// 读入风速文件
		double** VwMatrix= new double*[WindRows];
		for (int k=0; k<WindRows; k++)
		{
		...
		for (int fileIndex=1;fileIndex<=nFiles_GenVw;fileIndex++)
		{
			...
			// 过滤掉startTime-2*deltaT以前的行
			double t; // 时间
			if (fileIndex < nFiles_GenVw)//不是最后一个文件，直接读走第一行的10个数据（除去时间变量）
			{
				infile >> t;
				// 过滤掉startTime以前的数据
				double tmp = 0;
				while ( fabs(t-startTime+2*deltaT) > deltaT/2 )
				{
					for(int k=0;k<10;k++)
						infile >> tmp;
					infile >> t;
				...
				// 给风速存储数组赋值 
				for(int i=0;i<WindRows;i++)//read VwData
				{
					for(int j=0; j<10; j++)
			...
			else//到了最后一个文件，只需要读走units个变量即可
			{
				...
				// 给风速存储数组赋值 
				for(int i=0;i<WindRows;i++)//read 10 initdata
				{
		...
		for (int k=0; k<nIndGen; k++) {
			Component* tempCom = branches->at(branchNumArray_IndGen[k]);
			tempCom->getWindVelocityData(VwMatrix,WindRows,ptr);
		}
	...
	// 读取延时模块初值文件
	if ( nDelay + nLoopNodes != 0 )
	{
		int nCol_Delay = nDelay + nLoopNodes;
		int units_Delay = nCol_Delay%10;
		int nFiles_Delay = (nCol_Delay-units_Delay)/10;
		if(units_Delay!=0)
		{
			nFiles_Delay = nFiles_Delay + 1;
		}

		// 读入数据
		for (int fileIndex=1;fileIndex<=nFiles_Delay;fileIndex++)
		{
			int num1 = fileIndex / 10;
			int num2 = fileIndex % 10;
			char chDir[100];   
			string caseDir;
#ifdef WIN32
			sprintf(chDir,".\\result\\pscadData\\case%d_Ctrl_%d%d.out",caseNumber,num1,num2);   
#else
			sprintf(chDir,"..//result//pscadData//case%d_Ctrl_%d%d.out",caseNumber,num1,num2);   
#endif
			caseDir=chDir;
			...
			infile>>t;
			nR=(int)((startTime-t)/initDeltaT+0.5+1);
			// 延时模块初始化向量
			DelayInitialValue = new double* [nR];
			for (int i=0;i<nR;i++)
			{
				DelayInitialValue[i] = new double [nDelay];
			}

			if (fileIndex < nFiles_Delay)//不是最后一个文件，直接读走第一行的10个数据（除去时间变量）
			{
				for (int i=0;i<nR;i++)
				{
					for(int j=0; j<10; j++)
					{
						if (j+10*(fileIndex-1)<nDelay)
						{
							infile >> DelayInitialValue[i][j+10*(fileIndex-1)];
						}
						else
						{
							if (i==nR-1)
							{
								infile>>loopNodeValue[k3];
								k3++;
							}
							else
							{
	...
	{
		// 赋初值
		tempCtrlCom = ctrlBranches->at(DelayBranches[i]);
		tempCtrlCom->initializeDelay(DelayInitialValue, ptr, nR);
	}
...
{
	caseDfnMatrix.ResizeTo(20000,10);
	char ch[100];     
	string dir1;
#ifdef WIN32
...
void EMTP::	specifySystem()
{
	...
	WindRows=1;//读入风速数组的长度
	t = startTime;
	while(t<finishTime-deltaT/2)
	{
		t=t+deltaT;
		WindRows++;
	}
	...
	int diodeWithSCCount=0, Asych5Count=0;
	...
	{   
		...
		case 1: 
			elecCompCount[0] +=1;   
			com =new Resistance(elecCompCount[0],(int)caseDfnMatrix(i,1),(int)caseDfnMatrix(i,2),caseDfnMatrix(i,3));
			branches->push_back(com);
			break;
		case 2:
			elecCompCount[1]+=1;
			com =new Inductance(elecCompCount[1],(int)caseDfnMatrix(i,1),(int)caseDfnMatrix(i,2),caseDfnMatrix(i,3));
			branches->push_back(com);
			break;
		case 3:
			elecCompCount[2]+=1;
			com =new Capacitance(elecCompCount[2],(int)caseDfnMatrix(i,1),(int)caseDfnMatrix(i,2),caseDfnMatrix(i,3));
			branches->push_back(com);
			break;
		case 4:
			elecCompCount[3]+=1;
			com =new AcVoltageSource(elecCompCount[3],(int)caseDfnMatrix(i,1),(int)caseDfnMatrix(i,2),caseDfnMatrix(i,3),caseDfnMatrix(i,4),caseDfnMatrix(i,5),caseDfnMatrix(i,6));
			branches->push_back(com);
			break;
		case 5:
			elecCompCount[4]=elecCompCount[4]+1;
			com =new SinglePhaseTransformer(elecCompCount[4],nodeNumberArray,apparentPowerRating,voltageRating);
			branches->push_back(com);
			nColumns=nColumns+1;
			break;
		case 6:
			elecCompCount[5]=elecCompCount[5]+1;
		...
		case 7:
			elecCompCount[6] += 1;
			com =new Diode(elecCompCount[6],(int)caseDfnMatrix(i,1),(int)caseDfnMatrix(i,2),caseDfnMatrix(i,3),caseDfnMatrix(i,4),caseDfnMatrix(i,5));
			branches->push_back(com);
			break;
		case 8:
			elecCompCount[7] += 1;
			com = new TimeSwitch(elecCompCount[7],(int)caseDfnMatrix(i,1),(int)caseDfnMatrix(i,2),caseDfnMatrix(i,3),caseDfnMatrix(i,4),caseDfnMatrix(i,5),caseDfnMatrix(i,6));
			branches->push_back(com);
			break;
			//case 9:
			//	diodebridgeCount += 1;
		...
		case 11:
			elecCompCount[10] += 1;
			com = new Inverter(elecCompCount[10],(int)caseDfnMatrix(i,1),(int)caseDfnMatrix(i,2),(int)caseDfnMatrix(i,3),(int)caseDfnMatrix(i,4),(int)caseDfnMatrix(i,5),caseDfnMatrix(i,6),caseDfnMatrix(i,7));
			branches->push_back(com);
		...
		case 13:
			elecCompCount[12] += 1;
			com = new IGBT(elecCompCount[12],(int)caseDfnMatrix(i,1),(int)caseDfnMatrix(i,2),caseDfnMatrix(i,3),caseDfnMatrix(i,4),caseDfnMatrix(i,5),caseDfnMatrix(i,6));
			branches->push_back(com);
			break;
			//case 14:	//浙大的控制方法
		...
		case 17:
			elecCompCount[16] += 1;
			com = new ThreePhaseRectifierBridge(elecCompCount[16] ,in_ACNode,in_DCNode,in_state,1.0e-2,1.0e6);
			branches->push_back(com);
			nColumns=nColumns+5;
			break;
		case 18:
			elecCompCount[17]+=1;
			com=new WoundInducGenerator(elecCompCount[17],(int)caseDfnMatrix(i,1),(int)caseDfnMatrix(i,2),(int)caseDfnMatrix(i,3),caseDfnMatrix(i,4));
			branches->push_back(com);
			nColumns=nColumns+5;
			break;
		case 19:
		...
		case 20:
			elecCompCount[19]+=1;
			com =new TimedAcVoltageSource(elecCompCount[19],(int)caseDfnMatrix(i,1),(int)caseDfnMatrix(i,2),caseDfnMatrix(i,3),caseDfnMatrix(i,4),caseDfnMatrix(i,5),caseDfnMatrix(i,6),caseDfnMatrix(i,7),caseDfnMatrix(i,8),caseDfnMatrix(i,9));
			branches->push_back(com);
			break;
		case 21:
			elecCompCount[20]+=1;
			com=new Impedance(elecCompCount[20],(int)caseDfnMatrix(i,1),(int)caseDfnMatrix(i,2),caseDfnMatrix(i,3),caseDfnMatrix(i,4));
			branches->push_back(com);
			break;
		case 22:
			elecCompCount[21] += 1;
			com =new newIGBT(elecCompCount[21],(int)caseDfnMatrix(i,1),(int)caseDfnMatrix(i,2),caseDfnMatrix(i,3),caseDfnMatrix(i,4),caseDfnMatrix(i,5));
			branches->push_back(com);
			break;
		case 23:
			elecCompCount[22]+=1;
			com = new ControlledVoltageSource(elecCompCount[22],(int)caseDfnMatrix(i,1),(int)caseDfnMatrix(i,2),caseDfnMatrix(i,3),(int)caseDfnMatrix(i,4));
			branches->push_back(com);
			break;
		case 24:
			elecCompCount[23] += 1;
		...
		case 25:
			elecCompCount[24] += 1;
			com = new ThreePhaseTransformer(elecCompCount[24],(int)caseDfnMatrix(i,1),(int)caseDfnMatrix(i,2),caseDfnMatrix(i,3),caseDfnMatrix(i,4),caseDfnMatrix(i,5),caseDfnMatrix(i,6),caseDfnMatrix(i,7),caseDfnMatrix(i,8),caseDfnMatrix(i,9));
			branches->push_back(com);
			nColumns = nColumns + 6;
			break;
		case 26:
			elecCompCount[25]+=1;
			com = new PhotoVoltaic(elecCompCount[25],(int)caseDfnMatrix(i,1),(int)caseDfnMatrix(i,2),caseDfnMatrix(i,3),(int)caseDfnMatrix(i,4),(int)caseDfnMatrix(i,5),(int)caseDfnMatrix(i,6),(int)caseDfnMatrix(i,7));
			branches->push_back(com);
			break;
		case 27:
			elecCompCount[26]+=1;
			com = new TimedResistance(elecCompCount[26],(int)caseDfnMatrix(i,1),(int)caseDfnMatrix(i,2),caseDfnMatrix(i,3),caseDfnMatrix(i,4),caseDfnMatrix(i,5),caseDfnMatrix(i,6));
			branches->push_back(com);
			break;
		case 28:
			elecCompCount[27]+=1;
		...
		case 77:
			diodeWithSCCount += 1;
			com =new DiodeWithSC(diodeWithSCCount,(int)caseDfnMatrix(i,1),(int)caseDfnMatrix(i,2),caseDfnMatrix(i,3),caseDfnMatrix(i,4),caseDfnMatrix(i,5),caseDfnMatrix(i,6),caseDfnMatrix(i,7));
			branches->push_back(com);
			break;
		case 555:
			Asych5Count += 1;
	...
	nTimedResistance = elecCompCount[26];
	...
	//二极管相关
	nSwitch = elecCompCount[6]+elecCompCount[21];
	if (nSwitch != 0)
	{
	...
	//TimedResistance相关
	if (nTimedResistance != 0)
	{
		timedResistanceNumArray = new int[nTimedResistance];
	...
	nControlledBranches = elecCompCount[21]+elecCompCount[22]+elecCompCount[25];
	if (nControlledBranches != 0)
	{
		...
		{
			tempCom = branches->at(k);
			if(tempCom->type==22||tempCom->type==23||tempCom->type==26)
			{
	...
	/*          PWM变流器平均模型相关数组创建                    */
	/*************************************************************/
	nPWMConverter = elecCompCount[23];
	if ( nPWMConverter != 0 )
	{
...
{
	caseMsrDfnMatrix.ResizeTo(10000,3);
	char ch[100];     
	string dir1;
#ifdef WIN32
...
void EMTP::	specifyMsrSystem()
{
	...
	{   
		...
		case 3:
			...
			break;
		case 4:
			com = new InducGenWrMsr(i+1,(int)caseMsrDfnMatrix(i,1),(int)caseMsrDfnMatrix(i,2));//电机转速测量元件
			msrComps->push_back(com);
			break;
		case 5:
			com = new InducGenAngleMsr(i+1,(int)caseMsrDfnMatrix(i,1),(int)caseMsrDfnMatrix(i,2));//电机转子角测量元件
			msrComps->push_back(com);
			break;
...
{
	caseCtrlDfnMatrix.ResizeTo(20000,10);
	char ch[100];     
	string dir1;
#ifdef WIN32
...
void EMTP::	specifyCtrlSystem()
{
	...
	nLoopNodes=0;
	int rowCtrlDfnMatrix = nCtrlBranches;

	int ctrlCompTypeNumber =20;
	int* ctrlCompCount = new int[ctrlCompTypeNumber];
	for (int i=0;i<ctrlCompTypeNumber;i++)
	{
		ctrlCompCount[i] = 0;
	}

	for(int i=0;i<rowCtrlDfnMatrix;i++)
	{   
		...
		{
		case 1: 
			com = new SigmaCtrlComp(++ctrlCompCount[0],(int)caseCtrlDfnMatrix(i,1),(int)caseCtrlDfnMatrix(i,2),(int)caseCtrlDfnMatrix(i,3),(int)caseCtrlDfnMatrix(i,4));//加和器元件
			...
			break;
		case 2:
			com = new PICtrlComp(++ctrlCompCount[1],(int)caseCtrlDfnMatrix(i,1),(int)caseCtrlDfnMatrix(i,2),caseCtrlDfnMatrix(i,3),caseCtrlDfnMatrix(i,4));//PI元件
			ctrlBranches->push_back(com);
			nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,1));
			nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,2));
			break;
		case 3:
			com = new ConstantCtrlComp(++ctrlCompCount[2],(int)caseCtrlDfnMatrix(i,1),caseCtrlDfnMatrix(i,2));//常量输入元件
			ctrlBranches->push_back(com);
			nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,1));
			break;
		case 4:
			com = new PulseGenComp(++ctrlCompCount[3],(int)caseCtrlDfnMatrix(i,1),caseCtrlDfnMatrix(i,2),caseCtrlDfnMatrix(i,3),caseCtrlDfnMatrix(i,4),caseCtrlDfnMatrix(i,5),caseCtrlDfnMatrix(i,6));//脉冲波发生元件
			ctrlBranches->push_back(com);
			nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,1));
			break;
		case 5:
			com = new TriangleGenComp(++ctrlCompCount[4],(int)caseCtrlDfnMatrix(i,1),caseCtrlDfnMatrix(i,2),caseCtrlDfnMatrix(i,3),caseCtrlDfnMatrix(i,4),caseCtrlDfnMatrix(i,5),caseCtrlDfnMatrix(i,6));//三角波发生元件
			ctrlBranches->push_back(com);
			nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,1));
			break;
		case 6:
			com = new SinGenComp(++ctrlCompCount[5],(int)caseCtrlDfnMatrix(i,1),caseCtrlDfnMatrix(i,2),caseCtrlDfnMatrix(i,3),caseCtrlDfnMatrix(i,4));//正弦波发生元件
			ctrlBranches->push_back(com);
			nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,1));
			break;
		case 7: 
			com = new PCtrlComp(++ctrlCompCount[6],(int)caseCtrlDfnMatrix(i,1),(int)caseCtrlDfnMatrix(i,2),caseCtrlDfnMatrix(i,3));//比例元件
			ctrlBranches->push_back(com);
			nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,1));
			nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,2));
			break;
		case 8:
			com = new T2DTrans(++ctrlCompCount[7],(int)caseCtrlDfnMatrix(i,1),(int)caseCtrlDfnMatrix(i,2),(int)caseCtrlDfnMatrix(i,3),(int)caseCtrlDfnMatrix(i,4),(int)caseCtrlDfnMatrix(i,5));//3-2变换元件
			ctrlBranches->push_back(com);
			for (int j=1;j<6;j++)
			{
			...
			break;
		case 9:
			com = new D2TTrans(++ctrlCompCount[8],(int)caseCtrlDfnMatrix(i,1),(int)caseCtrlDfnMatrix(i,2),(int)caseCtrlDfnMatrix(i,3),(int)caseCtrlDfnMatrix(i,4),(int)caseCtrlDfnMatrix(i,5));//2-3变换元件
			ctrlBranches->push_back(com);
			for (int j=1;j<6;j++)
			{
				nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,j));
			...
			break;
		case 10:
			com = new S2RTrans(++ctrlCompCount[9],(int)caseCtrlDfnMatrix(i,1),(int)caseCtrlDfnMatrix(i,2),(int)caseCtrlDfnMatrix(i,3),(int)caseCtrlDfnMatrix(i,4),(int)caseCtrlDfnMatrix(i,5));//旋转坐标变换元件
			ctrlBranches->push_back(com);
			for (int j=1;j<6;j++)
			{
				nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,j));
			...
			break;
		case 11:
			com = new R2STrans(++ctrlCompCount[10],(int)caseCtrlDfnMatrix(i,1),(int)caseCtrlDfnMatrix(i,2),(int)caseCtrlDfnMatrix(i,3),(int)caseCtrlDfnMatrix(i,4),(int)caseCtrlDfnMatrix(i,5));//旋转坐标变换元件
			ctrlBranches->push_back(com);
			for (int j=1;j<6;j++)
			{
				nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,j));
			...
			break;
		case 12:
			com = new PRCoordinate(++ctrlCompCount[11],(int)caseCtrlDfnMatrix(i,1),(int)caseCtrlDfnMatrix(i,2),(int)caseCtrlDfnMatrix(i,3),(int)caseCtrlDfnMatrix(i,4),(int)caseCtrlDfnMatrix(i,5));//直角坐标-极坐标变换元件
			ctrlBranches->push_back(com);
			for (int j=1;j<5;j++)
			{
				nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,j));
			...
			break;
		case 13:
			com = new Comparator(++ctrlCompCount[12],(int)caseCtrlDfnMatrix(i,1),(int)caseCtrlDfnMatrix(i,2),(int)caseCtrlDfnMatrix(i,3),(int)caseCtrlDfnMatrix(i,4),caseCtrlDfnMatrix(i,5),caseCtrlDfnMatrix(i,6));//比较器元件
			ctrlBranches->push_back(com);
			for (int j=1;j<5;j++)
			{
				nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,j));
			...
			break;
		case 14:
			com = new Limiter(++ctrlCompCount[13],(int)caseCtrlDfnMatrix(i,1),(int)caseCtrlDfnMatrix(i,2),caseCtrlDfnMatrix(i,3),caseCtrlDfnMatrix(i,4));//限幅元件
			ctrlBranches->push_back(com);
			for (int j=1;j<3;j++)
			{
				nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,j));
			...
			break;
		case 15:
			com = new TimeConstant(++ctrlCompCount[14],(int)caseCtrlDfnMatrix(i,1));//时间常量元件
			ctrlBranches->push_back(com);
			nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,1));
			break;
		case 16:
			com = new ImpulseGenComp(++ctrlCompCount[15],(int)caseCtrlDfnMatrix(i,1),caseCtrlDfnMatrix(i,2),caseCtrlDfnMatrix(i,3),caseCtrlDfnMatrix(i,4),caseCtrlDfnMatrix(i,5));//脉冲输出元件
			ctrlBranches->push_back(com);
			nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,1));
			break;
		case 17:
			com = new Sampler(++ctrlCompCount[16],(int)caseCtrlDfnMatrix(i,1),(int)caseCtrlDfnMatrix(i,2),(int)caseCtrlDfnMatrix(i,3));//采样模块
			ctrlBranches->push_back(com);
			for (int j=1;j<4;j++)
			{
				nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,j));
			...
			break;
		case 18:
			com = new Delay(++ctrlCompCount[17],(int)caseCtrlDfnMatrix(i,1),(int)caseCtrlDfnMatrix(i,2),caseCtrlDfnMatrix(i,3),(int)caseCtrlDfnMatrix(i,4));//延迟模块
			ctrlBranches->push_back(com);
			for (int j=1;j<3;j++)
			{
				nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,j));
			...
			break;
		case 19:
			com = new Selector(++ctrlCompCount[18],(int)caseCtrlDfnMatrix(i,1),(int)caseCtrlDfnMatrix(i,2),(int)caseCtrlDfnMatrix(i,3),(int)caseCtrlDfnMatrix(i,4), caseCtrlDfnMatrix(i,5));//选择开关模块
			ctrlBranches->push_back(com);
			for (int j=1;j<5;j++)
			{
				nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,j));
			...
			break;
		case 20:
			com = new SampleHold(++ctrlCompCount[19],(int)caseCtrlDfnMatrix(i,1),(int)caseCtrlDfnMatrix(i,2),(int)caseCtrlDfnMatrix(i,3));//采样保持模块
			ctrlBranches->push_back(com);
			for (int j=1;j<4;j++)
			{
				nCtrlNodes = max(nCtrlNodes, (int)caseCtrlDfnMatrix(i,j));
			...
			break;
		case 100:
			nLoopNodes+=caseCtrlDfnMatrix(i,1);
			nCtrlBranches--;
			break;
	...
	//闭环节点数组生成
	loopNodeNumber = new int[nLoopNodes];
	loopNodeValue = new double[nLoopNodes];
	loopNodeValue_bak = new double [nLoopNodes];
	int k3=0;

	for (int i=0;i<rowCtrlDfnMatrix;i++)
	{
		if (caseCtrlDfnMatrix(i,0)==100)
		{
			for (int j=0;j<caseCtrlDfnMatrix(i,1);j++)
			{
				loopNodeNumber[k3] = caseCtrlDfnMatrix(i,j+2);
				k3++;
			}
		}
	}
	for (int i=0;i<nLoopNodes;i++)
	{
		loopNodeValue[i] = 0;
		loopNodeValue_bak[i] = 0;
	}
	...
	// xuyin, 20121110
	// PI Controller 相关数组创建
	nPIController = ctrlCompCount[1];
	if (nPIController != 0)
	{
	...
	// 延迟模块相关数组创建
	nDelay = ctrlCompCount[17];
	if (nDelay != 0)
	{
		DelayBranches = new int[nDelay];
		int k3=0;
		for (int k=0;k<nCtrlBranches;k++)
		{
			CtrlComponent* tempCtrlCom = ctrlBranches->at(k);
			if( tempCtrlCom->type == 18 )
			{
				DelayBranches[k3] = k;
				k3++;
			}
...
void EMTP::transferMeasurands()
{
	...
	for (int i=0;i<nMsrComps;i++)
	{
		...
		ctrlNodeValue[tempMsrComp->ctrlNode-1] = tempMsrComp ->getMeasurands(nodeVoltageVec,branches);
...
void EMTP::solveCtrlSystem()
{
	...
	for (int i=0;i<nLoopNodes;i++)
	{
		ctrlNodeValue[loopNodeNumber[i]-1] = loopNodeValue[i];
		nodeCalMark[loopNodeNumber[i]-1] = 1;
	}
		...
		for (int i=0;i<nCtrlBranches;i++)
		{
			...
			else
			{
				...
				if (tempCtrlComp->checkCalCondition(nodeCalMark)==1)
				{
					...
					tempCtrlComp->calculateOutputValue(curTime+deltaT);
	...
	for (int i=0;i<nLoopNodes;i++)
	{
		loopNodeValue[i]=ctrlNodeValue[loopNodeNumber[i]-1];
	}
...
void EMTP::solveG(double * tau_new)
{
	...
	for (int k=0; k<Ns; k++)
	{
		...
		//若存在时变电阻，则每个时步都要检测其状态是否发生变化。若变化，则需要重新形成导纳矩阵。
		if (nTimedResistance)
		{
			int timedResistanceStatus=0;
			for (int k=0;k<nTimedResistance;k++)
			{
				Component* tempCom = branches->at(timedResistanceNumArray[k]);
				if (tempCom->timedResistanceStatusChange(curTime))
				{
					tempCom->calculateNortonEquivalentResistance(curTime);
					timedResistanceStatus++;
				}
			}
			if (timedResistanceStatus)
			{
				formConductanceMatrix();
			}
		}
		...
		calculateBranchNortonEquivalentCurrent(curTime); //计算支路诺顿等效电流
		//my_clock(tEnd);
		//t1+=cal_timeCost(tStart,tEnd);

		//my_clock(tStart);
		formNodeNortonEquivalentCurrentArray(); //形成节点诺顿等效电流向量
		//my_clock(tEnd);
		//t2+=cal_timeCost(tStart,tEnd);

		//my_clock(tStart);
		solveNodeVoltageEquation(); //求解节点电压方程
		//my_clock(tEnd);
		//t3+=cal_timeCost(tStart,tEnd);
		...
		calculateBranchCurrent(); //计算支路电流
		my_clock(tEnd);
		//t4+=cal_timeCost(tStart,tEnd);
		
		t5+=cal_timeCost(tStart,tEnd);
		...
		my_clock(tEnd);
		t6+=cal_timeCost(tStart,tEnd);

		// 对于272算例，试图模拟二极管桥导通和关断动态
		/*if (nodeVoltageVec(4)-nodeVoltageVec(5)<0 && DiodeMark==0)
		{
			DiodeMark =1;
			cout<<"Diode bridge is on at"<<curTime<<endl;
			conductanceMatrix(4,4)+=1500;
			conductanceMatrix(5,5)+=1500;
			conductanceMatrix(4,5)-=1500;
			conductanceMatrix(5,4)-=1500;
			lu_conductanceMatrix->SetMatrix(conductanceMatrix);
			lu_conductanceMatrix->Decompose();
		}
		else if (nodeVoltageVec(4)-nodeVoltageVec(5)>0 && DiodeMark==1)
		{
			DiodeMark =0;
			cout<<"Diode bridge is off at"<<curTime<<endl;
			conductanceMatrix(4,4)-=1500;
			conductanceMatrix(5,5)-=1500;
			conductanceMatrix(4,5)+=1500;
			conductanceMatrix(5,4)+=1500;
...
void EMTP::store_state()
{
	...
	for (int k=0;k<nLoopNodes;k++)
	{
		loopNodeValue_bak[k] = loopNodeValue[k];
	}
}
...
void EMTP::restore_state()
{
	...
	for (int k=0;k<nLoopNodes;k++)
	{
		loopNodeValue[k] = loopNodeValue_bak[k];
	}
...
//系统计时相关
void EMTP::my_clock(LARGE_INTEGER &t)//计时
{
	QueryPerformanceCounter(&t);
}
double EMTP::cal_timeCost(LARGE_INTEGER &t_start,LARGE_INTEGER &t_end)//计算耗时，返回值单位为us
{
